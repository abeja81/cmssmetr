...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.invitation.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.invitations.update", [$invitation->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('event') ? 'is-invalid' : '' }}" name="event_id" id="event_id">
                    @foreach($events as $id => $entry)
                        <option value="{{ $id }}" {{ (old('event_id') ? old('event_id') : $invitation->event->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('event'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('all_users_invites') ? 'is-invalid' : '' }}" name="all_users_invites[]" id="all_users_invites" multiple>
                    @foreach($all_users_invites as $id => $all_users_invite)
                        <option value="{{ $id }}" {{ (in_array($id, old('all_users_invites', [])) || $invitation->all_users_invites->contains($id)) ? 'selected' : '' }}>{{ $all_users_invite }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('invite_sms') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="invite_sms" id="invite_sms" value="1" {{ $invitation->invite_sms || old('invite_sms', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('invite_email') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="invite_email" id="invite_email" value="1" {{ $invitation->invite_email || old('invite_email', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('invite_whatsapp') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="invite_whatsapp" id="invite_whatsapp" value="1" {{ $invitation->invite_whatsapp || old('invite_whatsapp', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('invitation_valid_date') ? 'is-invalid' : '' }}" type="text" name="invitation_valid_date" id="invitation_valid_date" value="{{ old('invitation_valid_date', $invitation->invitation_valid_date) }}">
